...
// =====================================================================
{
  ...
  theory: {
    concepts: [
      ...
      { heading: "11. Columnas IDENTITY (autoincremento nativo)",
        explanation: "Desde Oracle 12c, una columna puede autonumerarse sin necesitar una secuencia y un trigger manuales. GENERATED ALWAYS AS IDENTITY hace que Oracle rechace cualquier INSERT que intente fijar un valor explícito para esa columna, salvo que se use INSERT ... OVERRIDING SYSTEM VALUE. GENERATED BY DEFAULT AS IDENTITY sí permite indicar un valor propio en el INSERT, y solo genera uno automático cuando se omite. GENERATED BY DEFAULT ON NULL AS IDENTITY además genera un valor automático cuando se inserta explícitamente NULL en esa columna. Internamente Oracle sigue creando una secuencia asociada a la columna (admite START WITH e INCREMENT BY), pero ya no hace falta crearla ni mantenerla a mano como en el patrón clásico secuencia + trigger.",
        syntax: "columna NUMBER GENERATED { ALWAYS | BY DEFAULT [ON NULL] } AS IDENTITY\n  [(START WITH n INCREMENT BY n)]",
        examples: [
          { code: "CREATE TABLE clientes (\n  id_cliente  NUMBER GENERATED ALWAYS AS IDENTITY,\n  nombre      VARCHAR2(50) NOT NULL\n);\n\nINSERT INTO clientes (nombre) VALUES ('Ana López');", output: "id_cliente se genera automáticamente (1, 2, 3...); no se indica en el INSERT" },
          { code: "INSERT INTO clientes (id_cliente, nombre)\nOVERRIDING SYSTEM VALUE\nVALUES (500, 'Cliente migrado');", output: "Necesario con GENERATED ALWAYS si se quiere forzar un valor concreto, por ejemplo al migrar datos históricos" }
        ],
        commonErrors: [
          "Intentar INSERT INTO clientes (id_cliente, nombre) VALUES (1, ...) sobre una columna GENERATED ALWAYS AS IDENTITY sin OVERRIDING SYSTEM VALUE: Oracle lanza ORA-32795.",
          "Confundir GENERATED ALWAYS (nunca acepta valor explícito sin OVERRIDING) con GENERATED BY DEFAULT (sí acepta un valor explícito con normalidad)."
        ] },
      { heading: "12. Columnas invisibles",
        explanation: "Desde Oracle 12c, una columna puede marcarse como INVISIBLE: sigue existiendo, se puede consultar y actualizar si se la nombra explícitamente, pero desaparece de SELECT *, de DESCRIBE, y no hace falta darle un valor en un INSERT sin lista de columnas. Es útil para añadir una columna nueva a una tabla en producción sin romper aplicaciones existentes que hacen SELECT * o INSERT sin lista de columnas, hasta que esas aplicaciones estén preparadas para usarla. Se declara INVISIBLE al crear la tabla, o se activa después con ALTER TABLE ... MODIFY (columna INVISIBLE), y se revierte con VISIBLE.",
        syntax: "columna tipo_dato INVISIBLE\nALTER TABLE tabla MODIFY (columna INVISIBLE | VISIBLE)",
        examples: [
          { code: "ALTER TABLE clientes ADD (fecha_baja DATE INVISIBLE);\n\nSELECT * FROM clientes;", output: "fecha_baja NO aparece en el resultado de SELECT *" },
          { code: "SELECT id_cliente, nombre, fecha_baja FROM clientes;", output: "fecha_baja SÍ aparece si se nombra explícitamente en el SELECT" }
        ],
        commonErrors: [
          "Pensar que una columna invisible no existe o no se puede consultar: solo se oculta de SELECT * y DESCRIBE, sigue siendo consultable por nombre.",
          "Olvidar que un INSERT sin lista de columnas no necesita (ni puede) dar un valor a una columna invisible, salvo que se la incluya expresamente en la lista de columnas del INSERT."
        ] }
    ],
    oracleNotes: [
      ...
      "GENERATED ALWAYS AS IDENTITY rechaza valores explícitos (ORA-32795) salvo que se use OVERRIDING SYSTEM VALUE; GENERATED BY DEFAULT los admite con normalidad.",
      "Una columna INVISIBLE sigue siendo consultable y actualizable por nombre; solo desaparece de SELECT *, DESCRIBE e INSERT sin lista de columnas."
    ]
  },
  summary: [
    ...
    "Las columnas IDENTITY autonumeran sin secuencia ni trigger manuales (ALWAYS no admite valor explícito salvo OVERRIDING SYSTEM VALUE; BY DEFAULT sí lo admite); las columnas INVISIBLE se ocultan de SELECT */DESCRIBE pero siguen siendo consultables por nombre."
  ],
  comparisonTable: {
  ...
  mindMap: [
    { topic: "Módulo 12 — DDL de tablas", children: [
      ...
      "Autonumeración → IDENTITY (ALWAYS vs BY DEFAULT), reemplaza secuencia+trigger",
      "Visibilidad → columna INVISIBLE (oculta de SELECT */DESCRIBE, consultable por nombre)"
    ] }
  ],
  sourceRefs: [
  ...
  mistakes: [
    ...
    { mistake: "Intentar insertar un valor explícito en una columna GENERATED ALWAYS AS IDENTITY sin OVERRIDING SYSTEM VALUE.", why: "El examen presenta un INSERT con valor explícito sobre una columna ALWAYS esperando que sepas que provoca ORA-32795, a diferencia de BY DEFAULT, que sí lo permitiría." },
    { mistake: "Creer que una columna INVISIBLE desaparece por completo de la tabla.", why: "El examen pregunta qué devuelve SELECT * frente a un SELECT que nombra la columna explícitamente, para comprobar si sabes que solo se oculta, no se elimina." }
  ],
  exercises: [
    ...
    { title: "Autonumerar sin secuencia manual", difficulty: "intermedio", prompt: "Crea una tabla pedidos con id_pedido autonumerado de forma que nunca se pueda insertar un valor manual salvo que se fuerce explícitamente, y una columna fecha DATE.", hint: "GENERATED ALWAYS AS IDENTITY", solution: "CREATE TABLE pedidos (\n  id_pedido NUMBER GENERATED ALWAYS AS IDENTITY,\n  fecha DATE\n);" },
    { title: "Añadir una columna sin romper el código existente", difficulty: "avanzado", prompt: "Añade a la tabla clientes una columna fecha_baja DATE que no aparezca en los SELECT * ni haga falta indicarla en los INSERT sin lista de columnas que ya usa la aplicación en producción.", hint: "INVISIBLE", solution: "ALTER TABLE clientes ADD (fecha_baja DATE INVISIBLE);" }
  ],
  solved: [
  ...
  flashcards: [
    ...
    { front: "¿Qué diferencia hay entre GENERATED ALWAYS y GENERATED BY DEFAULT AS IDENTITY?", back: "ALWAYS rechaza valores explícitos (salvo OVERRIDING SYSTEM VALUE); BY DEFAULT los admite con normalidad." },
    { front: "¿Se puede seguir consultando una columna INVISIBLE?", back: "Sí, nombrándola explícitamente; solo se oculta de SELECT *, DESCRIBE e INSERT sin lista de columnas." }
  ],
  examples: [
  ...
  quiz: [
    ...
    { q: "¿Qué ocurre si se ejecuta un INSERT con un valor explícito para una columna GENERATED ALWAYS AS IDENTITY sin usar OVERRIDING SYSTEM VALUE?", options: ["Se inserta con normalidad", "Oracle lanza un error (ORA-32795)", "El valor se ignora y se autonumera igualmente", "La columna se convierte automáticamente en BY DEFAULT"], a: 1,
      why: [
        "No se inserta con normalidad: GENERATED ALWAYS bloquea valores explícitos por diseño.",
        "Correcta: Oracle rechaza el INSERT con ORA-32795 salvo que se use OVERRIDING SYSTEM VALUE.",
        "El valor no se ignora silenciosamente; la sentencia falla con error.",
        "Oracle no cambia el modo de la columna automáticamente; seguiría siendo ALWAYS."
      ] },
    { q: "¿Qué le ocurre a una columna marcada como INVISIBLE?", options: [
        "Deja de existir en la tabla", "Se oculta de SELECT * y DESCRIBE, pero se puede consultar nombrándola explícitamente",
        "Solo el DBA puede leerla", "Se convierte automáticamente en una columna virtual"
      ], a: 1,
      why: [
        "No deja de existir: sigue almacenada y se puede consultar y actualizar.",
        "Correcta: se oculta de SELECT * y DESCRIBE, pero es consultable si se la nombra explícitamente.",
        "Cualquier usuario con privilegios normales sobre la tabla puede consultarla si la nombra; no es exclusivo del DBA.",
        "No tiene relación con columnas virtuales: sigue siendo una columna normal almacenada, solo cambia su visibilidad por defecto."
      ] }
  ],
  challenges: [
...
// =====================================================================
{
  ...
  theory: {
    concepts: [
      ...
      { heading: "6. Generar JSON desde datos relacionales: JSON_OBJECT y JSON_ARRAY",
        explanation: "IS JSON, JSON_VALUE y JSON_TABLE parten de un JSON ya almacenado para consultarlo; JSON_OBJECT y JSON_ARRAY hacen el camino inverso: construyen un documento JSON a partir de columnas y valores relacionales normales, útil para exponer el resultado de una consulta SQL directamente en formato JSON (por ejemplo, para una API o un servicio web). JSON_OBJECT(clave VALUE valor, ...) crea un objeto {...} con esos pares clave-valor; JSON_ARRAY(valor1, valor2, ...) crea un array [...] con esos valores, y ambas se pueden anidar entre sí. Por defecto, un valor NULL sí se incluye en el objeto resultante (NULL ON NULL); ABSENT ON NULL omite esa clave por completo cuando su valor es NULL.",
        syntax: "JSON_OBJECT(clave VALUE valor [, clave VALUE valor ...] [ABSENT ON NULL | NULL ON NULL])\nJSON_ARRAY(valor1 [, valor2, ...] [ABSENT ON NULL | NULL ON NULL])",
        examples: [
          { code: "SELECT JSON_OBJECT('id' VALUE employee_id, 'nombre' VALUE last_name, 'salario' VALUE salary) AS empleado_json\nFROM   employees\nWHERE  employee_id = 100;", output: "{\"id\":100,\"nombre\":\"King\",\"salario\":24000}" },
          { code: "SELECT JSON_ARRAY(last_name, job_id, salary) AS fila_json\nFROM   employees\nWHERE  employee_id = 100;", output: "[\"King\",\"AD_PRES\",24000]" }
        ],
        commonErrors: [
          "Confundir el sentido de las funciones: JSON_TABLE va de JSON a relacional; JSON_OBJECT/JSON_ARRAY van de relacional a JSON, la dirección contraria.",
          "Olvidar que por defecto un valor NULL SÍ se incluye en el objeto (NULL ON NULL es el comportamiento habitual, salvo que se indique ABSENT ON NULL para omitir esas claves)."
        ] }
    ],
    oracleNotes: [
      ...
      "JSON_OBJECT y JSON_ARRAY generan JSON desde filas relacionales; son el camino inverso a JSON_TABLE, que convierte JSON en filas relacionales."
    ]
  },
  summary: [
    ...
    "JSON_OBJECT y JSON_ARRAY generan un documento JSON a partir de columnas relacionales: el camino inverso a JSON_TABLE."
  ],
  comparisonTable: {
    ...
    rows: [
      ...
      ["JSON_OBJECT / JSON_ARRAY", "Relacional → JSON", "Un documento JSON construido a partir de columnas y valores"]
    ]
  },
  mindMap: [
    { topic: "Módulo 19 — PIVOT/UNPIVOT y JSON", children: [
      ...
      "JSON_OBJECT / JSON_ARRAY → genera JSON desde columnas relacionales (camino inverso a JSON_TABLE)"
    ] }
  ],
  sourceRefs: [
  ...
  mistakes: [
    ...
    { mistake: "Pensar que JSON_TABLE también sirve para generar JSON a partir de una tabla relacional.", why: "JSON_TABLE solo lee JSON existente y lo convierte en filas; para el camino inverso (relacional a JSON) el examen espera que reconozcas JSON_OBJECT o JSON_ARRAY, no JSON_TABLE." }
  ],
  exercises: [
    ...
    { title: "Generar JSON desde una fila relacional", difficulty: "intermedio", prompt: "Construye un documento JSON con las claves 'id', 'nombre' y 'salario' a partir de employee_id, last_name y salary del empleado con employee_id = 100.", hint: "JSON_OBJECT('clave' VALUE columna, ...)", solution: "SELECT JSON_OBJECT('id' VALUE employee_id, 'nombre' VALUE last_name, 'salario' VALUE salary) AS empleado_json\nFROM employees\nWHERE employee_id = 100;" }
  ],
  solved: [
  ...
  flashcards: [
    ...
    { front: "¿Qué hacen JSON_OBJECT y JSON_ARRAY?", back: "Generan un documento JSON a partir de columnas y valores relacionales: el camino inverso a JSON_TABLE." },
    { front: "¿Un valor NULL se incluye por defecto en un JSON_OBJECT?", back: "Sí (NULL ON NULL es el comportamiento por defecto); ABSENT ON NULL omite esa clave." }
  ],
  examples: [
    ...
    { title: "JSON_OBJECT", code: "SELECT JSON_OBJECT('id' VALUE employee_id, 'nombre' VALUE last_name) AS empleado_json\nFROM employees\nWHERE employee_id = 100;" }
  ],
  quiz: [
    ...
    { q: "¿Qué función genera un documento JSON a partir de columnas relacionales, en el sentido contrario a JSON_TABLE?", options: ["JSON_VALUE", "JSON_QUERY", "JSON_OBJECT", "IS JSON"], a: 2,
      why: [
        "JSON_VALUE extrae un escalar de un JSON existente; no genera JSON desde datos relacionales.",
        "JSON_QUERY extrae una estructura (objeto/array) de un JSON existente; tampoco genera JSON desde cero.",
        "Correcta: JSON_OBJECT construye un objeto JSON a partir de pares clave-valor relacionales.",
        "IS JSON solo valida si un texto es JSON; no genera ningún documento."
      ] },
    { q: "Por defecto, ¿qué hace JSON_OBJECT cuando uno de los valores indicados es NULL?", options: [
        "Omite automáticamente esa clave del objeto", "Incluye la clave con valor NULL (comportamiento NULL ON NULL por defecto)",
        "Lanza un error de sintaxis", "Sustituye el NULL por la cadena vacía ''"
      ], a: 1,
      why: [
        "No la omite por defecto; eso requiere indicar explícitamente ABSENT ON NULL.",
        "Correcta: el comportamiento predeterminado es NULL ON NULL, así que la clave se incluye con valor NULL.",
        "No lanza ningún error: NULL es un valor válido para incluir en el objeto.",
        "No sustituye el NULL por una cadena vacía; lo representa como NULL en el JSON generado."
      ] }
  ],
  challenges: [
